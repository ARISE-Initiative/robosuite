...
**robosuite** is a robotic simulation framework powered by the [MuJoCo physics engine](http://mujoco.org/) for accessible and reproducible robot learning research. The current release focuses on robot manipulation. **robosuite** is part of the broader Advancing Robot Intelligence through Simulated Environments (ARISE) Initiative.
