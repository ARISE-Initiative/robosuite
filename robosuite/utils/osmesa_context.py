...
if not PYOPENGL_PLATFORM:
    ...
    raise ImportError(
        ...
        "(should be either unset or 'osmesa').".format(PYOPENGL_PLATFORM)
    )
...
class OSMesaGLContext:
    ...
    def make_current(self):
        if self._context:
            success = osmesa.OSMesaMakeCurrent(self._context, self._buffer, GL.GL_FLOAT, self._width, self._height)
            if not success:
