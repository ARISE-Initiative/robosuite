...
class OperationalSpaceAugmentedController(Controller):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        # Get Jacobians for each sensor site
        self.sensor_site_names = ["sensor_corner{0}".format(n) for n in range(1,7)]
        self.xpos_sensor = [np.ones(3)]*6
        self.J_pos_sensor = [np.zeros([3,7])]*6
    ...
    def run_controller(self):
        ...
        # Get nearest contact location to each geom
        sensor_contact_dict = {}

        active_contacts = self.sim.data.contact[:self.sim.data.ncon]
        for c in active_contacts:
            if (self.is_corner(c.geom1)):
                # idenfity which geom
                site_name = self.sim.model.geom_id2name(c.geom1)
                site_idx = int(re.search('corner(.+?)', site_name).group(1))-1
                if (site_idx in sensor_contact_dict):
                  new_dist = np.linalg.norm(np.array(c.pos) - self.xpos_sensor[site_idx])
                  if new_dist < np.linalg.norm(sensor_contact_dict[site_idx]):
                      sensor_contact_dict[site_idx] = c.pos
                else:
                  sensor_contact_dict[site_idx] = np.array(c.pos) - self.xpos_sensor[site_idx]

        # Compute forces from contacts along robot arm
        repel_torques = np.zeros(7)
        for sensor_idx, pos_vec in sensor_contact_dict.items():
          repel_force = -5*pos_vec/np.linalg.norm(pos_vec)
          #d = np.linalg.norm(pos_vec)
          #do = 0.002
          #repel_force = 1/1000*(1/d - 1/do)*1/d**2*pos_vec
          repel_torques += np.dot(self.J_pos_sensor[sensor_idx].T, repel_force)
        ...
        self.torques = np.dot(self.J_full.T, decoupled_wrench) + self.torque_compensation + repel_torques
    ...
    def update(self, force=False):
        ...
        # get Jacobians for sensor locations
        for i, site_name in enumerate(self.sensor_site_names):
          self.xpos_sensor[i] = self.sim.data.get_site_xpos("gripper0_" + site_name)
          self.J_pos_sensor[i] = self.sim.data.get_site_jacp("gripper0_" + site_name).reshape((3, -1))[:, self.qvel_index]
    ...
    def is_corner(self, geom_id):
        return "sensor_corner" in self.sim.model.geom_id2name(geom_id)
