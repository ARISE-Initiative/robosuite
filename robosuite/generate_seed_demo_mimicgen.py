...
def episode_to_mg_core(h5_group, obs_list, actions, env_name, cameras, H, W, control_freq, xml_model_str):
    ...
    for k in other_keys:
        if k.endswith("image") or k in og:
             continue
        try:
...
# (Modified to use MimicGen-compatible settings)
#
# ==============================================================================
...
class CerealSeedGenerator:
    ...
    def __init__(self, tmp_directory):
        ...
        self.config = {
            ...
            # Pass converter info into env_args
        ...
        self.env = suite.make(
            **self.config,
            ...
            ignore_done=True,
            use_camera_obs=True,
            camera_names=self.camera_names,     # <-- MIMICGEN CHANGE
            camera_heights=self.camera_height,  # <-- MIMICGEN CHANGE
            camera_widths=self.camera_width,    # <-- MIMICGEN CHANGE
            control_freq=self.control_freq,
        )
    ...
    def run_trajectory(self):
        ...
        video_path = os.path.join(self.tmp_directory, f"{ep_name}_video.mp4")
...
if __name__ == "__main__":
    ...
    generator = None # To store the last generator instance
    ...
    try:
        ...
        convert_episodes_root_to_hdf5(
            ...
            env_config["camera_height"],
            env_config["camera_width"]
        )
    except Exception as e:
