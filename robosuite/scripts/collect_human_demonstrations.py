...
def collect_human_trajectory(env, device, arm, env_configuration):
    ...
    while True:
        ...
        obs, _, _, _ = env.step(action, render=True)
        # print(obs)
...
if __name__ == "__main__":
    ...
    # controller position limits
    if 'PickPlace' in args.environment:
    ...
    elif args.environment == 'PegInHole':
        controller_config.update(dict(
            position_limits=[
                [-0.30, -0.30, 0.75],
                [0.15, 0.30, 1.00]
    ...
    # table offsets
    if 'PickPlace' in args.environment:
        config['bin1_pos'] = [0.0, -0.25, 0.8]
        config['bin2_pos'] = [0.0, 0.28, 0.8]
    elif args.environment == 'Wipe':
        config['table_offset'] = [0.05, 0, 0.8]
    else:
        config['table_offset'] = [-0.075, 0, 0.8]

    # env specific settings
    if args.environment == 'Door':
        skill_config['base_config']['lift_height'] = 1.15
        skill_config['base_config']['global_xyz_bounds'] = [[-0.25, -0.25, 0.90], [0.05, 0.05, 1.20]]
    ...
    env = GymWrapper(env, keys=['robot0_eef_pos', 'robot0_eef_quat', 'robot0_gripper_qpos', 'object-state'])
