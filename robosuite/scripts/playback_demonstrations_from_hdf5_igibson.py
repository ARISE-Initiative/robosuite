...
if __name__ == '__main__':
    ...
    writer = imageio.get_writer('video.mp4', fps=30)
    ...
    env_args['env_kwargs']['camera_names'] = 'frontview'
    ...
    if args.use_actions:
        ...
        for j, action in enumerate(actions):
            ...
            writer.append_data(obs['frontview_image'])
            image_count += 1
        ...
        for state in states:
            ...
            env.sim.forward()
            obs, _, _, _ = env.step(np.zeros(14))
            writer.append_data(obs['frontview_image'])
            image_count += 1
            i += 1

            camera_pos, camera_quat = rotate_camera(env, camera_pos, camera_quat)
            env.viewer._set_camera_pos_quat(camera_pos, camera_quat)
