...
class Bimanual(Manipulator):
    ...
    def _load_controller(self):
        ...
        for arm in self.arms:
            ...
            (start, end) = (
                (None, self._joint_split_idx) if arm == "right" else (self._joint_split_idx, None)
            )
    ...
    def reset(self, deterministic=False):
        ...
        if not deterministic:
            ...
            for arm in self.arms:
                if self.has_gripper[arm]:
                    self.sim.data.qpos[self._ref_gripper_joint_pos_indexes[arm]] = self.gripper[
                        arm
                    ].init_qpos
    ...
    def control(self, action, policy_step=False):
        ...
        for arm in self.arms:
            ...
            (start, end) = (
                (None, self._action_split_idx) if arm == "right" else (self._action_split_idx, None)
            )
        ...
        if policy_step:
            ...
            for arm in self.arms:
                ...
                self.recent_ee_pose[arm].push(
                    np.concatenate(
                        (self.controller[arm].ee_pos, T.mat2quat(self.controller[arm].ee_ori_mat))
                    )
                )
                self.recent_ee_vel[arm].push(
                    np.concatenate(
                        (self.controller[arm].ee_pos_vel, self.controller[arm].ee_ori_vel)
                    )
                )
