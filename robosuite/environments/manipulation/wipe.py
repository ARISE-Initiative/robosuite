...
class Wipe(SingleArmEnv):
    ...
    def reward(self, action=None):
        ...
        if self.check_contact(self.robots[0].robot_model):
            ...
            t1 = np.array(
                [np.dot(corner1_pos - corner2_pos, v1), np.dot(corner1_pos - corner2_pos, v2)]
            )
            t2 = np.array(
                [np.dot(corner2_pos - corner2_pos, v1), np.dot(corner2_pos - corner2_pos, v2)]
            )
            t3 = np.array(
                [np.dot(corner3_pos - corner2_pos, v1), np.dot(corner3_pos - corner2_pos, v2)]
            )
            t4 = np.array(
                [np.dot(corner4_pos - corner2_pos, v1), np.dot(corner4_pos - corner2_pos, v2)]
            )
        ...
        if self.use_object_obs:
            ...
            if self.use_condensed_obj_obs:
                ...
                def wipe_centroid(obs_cache):
                    return (
                        obs_cache["wipe_centroid"] if "wipe_centroid" in obs_cache else np.zeros(3)
                    )
