...
from robosuite.models.arenas import PushTArena
from robosuite.models.objects import TBarObject, CylinderObject
...
class PushT(ManipulationEnv):
    """
    This class corresponds to the push T task for a single robot arm.
    ...
    def _load_model(self):
        ...
        self.t_bar = TBarObject(name="tbar")

        # Create placement initializer
        if self.placement_initializer is not None:
            self.placement_initializer.reset()
            self.placement_initializer.add_objects(self.t_bar)
        else:
        ...
        for robot in self.robots:
            # Add peg to the robot's right arm
            eef0 = robot.robot_model.eef_name["right"]
            model0 = robot.robot_model
            body = find_elements(root=model0.worldbody, tags="body", attribs={"name": eef0}, return_first=True)
            body.append(peg_obj)
    ...
    def _setup_references(self):
        ...
        self.t_bar_body_id = self.sim.model.body_name2id(self.t_bar.root_body)
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def cube_pos(obs_cache):
                return np.array(self.sim.data.body_xpos[self.t_bar_body_id])

            @sensor(modality=modality)
            def cube_quat(obs_cache):
                return convert_quat(np.array(self.sim.data.body_xquat[self.t_bar_body_id]), to="xyzw")
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(gripper=self.robots[0].gripper, target=self.t_bar)
