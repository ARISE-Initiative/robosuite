"""Unitree G1 43-DoF robot model with Dex-3 hands."""
...
ASSET_DIR = Path(robosuite.models.assets_root) / "robots/g1_43dof"
XML_PATH = xml_path_completion("robots/g1_43dof/robot.xml")
...
def _actuator_names(joint_names: list[str]) -> list[str]:
    return [
        name[: -len("_joint")] if name.endswith("_joint") else name
        for name in joint_names
    ]


class G143DoF(LeggedManipulatorModel):
    """Unitree G1 43-DoF MuJoCo model with Dex-3 hands."""
